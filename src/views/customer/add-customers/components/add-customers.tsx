...
const FormSchema = z.object({
  ...
  calendar_date: z.union([z.date(), z.string()]).nullable(),
...
const AddCustomers = () => {
  ...
  const [createItem, { isSuccess: crateSuccess, isError: errorCreate, error }] = useCreateItemMutation();
  ...
  const onSubmit = async (data: FormValues) => {
    const reqData = {
      ...
      payment_type: data.payment_type ? JSON.parse(data.payment_type)?.id : '',
      payment_method: data.payment_method ? JSON.parse(data.payment_method)?.id : '',
      extra_phone: data.extra_phone ?? '',
      telegram_phone: data.telegram_phone ?? '',
      calendar_date: data.calendar_date ? (data.calendar_date === '' ? null : data.calendar_date) : null,
      comment_text: data.comment_text ?? '',
    };
    await createItem(reqData).unwrap();
  }
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          <span>{field.value ? new Date(field.value).toLocaleDateString() : "Kelish vaqti"}</span>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col mt-5">
                ...
                <Textarea className="w-full" placeholder="Izoh" {...field} />
                <FormMessage className="p-0 m-0" />
              </FormItem>
            )}
          />
