...
import { useDeletePaymentOrderListsItemMutation, useGetItemsPaymentQuery } from "../../../features/settings/payment-methods-lists";
...
const PaymentMetodTable = ({ activeTab, setActiveTab }: any) => {
    const [currentPage, setCurrentPage] = useState(1);
    const { data: dataPaymentMetod, isSuccess: paymentMetodSuccess, isLoading: isFetching, isError, error } = useGetItemsPaymentQuery(currentPage);
    const pageCount = dataPaymentMetod?.count ? Math.ceil(Number(dataPaymentMetod.count) / 10) : 1;
    const [deletePaymentOrderListsItem] = useDeletePaymentOrderListsItemMutation();
    ...
    const onConfirm = async () => {
        if (itemToDelete !== null) {
            try {
                await deletePaymentOrderListsItem({ id: itemToDelete });
                ...
                <div className="w-[100%]">
                    <Card>
                        <Table>
                            ...
                            <TableBody>
                                {
                                    paymentMetodSuccess && dataPaymentMetod.results.map((item: FilterResult) => (
                    ...
                    {
                        paymentMetodSuccess && dataPaymentMetod?.results?.length === 0 && (
                            <>
...
export default PaymentMetodTable;
