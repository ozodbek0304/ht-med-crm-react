...
import { useDeletePaymentTypeListsItemMutation, useGetItemsPaymentOrdersQuery } from "@/features/settings/payment-type-lists";
...
const PaymentTypeTable = ({ activeTab, setActiveTab }: any) => {
    const [currentPage, setCurrentPage] = useState(1);
    const { data: dataPaymentType, isSuccess: paymentTypeSuccess, isLoading: isFetching, isError, error } = useGetItemsPaymentOrdersQuery(currentPage);
    const [deletePaymentTypeListsItem] = useDeletePaymentTypeListsItemMutation();
    const pageCount = dataPaymentType?.count ? Math.ceil(Number(dataPaymentType.count) / 10) : 1;
    ...
    const onConfirm = async () => {
        if (itemToDelete !== null) {
            try {
                await deletePaymentTypeListsItem({ id: itemToDelete });
                ...
                <div className="w-[100%]">
                    <Card>
                        <Table>
                            ...
                            <TableBody>
                                {
                                    paymentTypeSuccess && dataPaymentType.results.map((item: FilterResult) => (
                    ...
                    {
                        paymentTypeSuccess && dataPaymentType?.results?.length === 0 && (
                            <>
